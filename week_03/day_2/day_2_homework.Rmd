...
#glimpse(face_descriptions)
```


```{r}
View(face_descriptions)
#slice_head(face_descriptions, n = 10) # get first 10 rows
```
...
face_descriptions_clean <- clean_names(face_descriptions)

names(face_descriptions_clean)
# Data variables (columns) names are acceptable
# data is in mixed case
...
```{r}
face_description_to_lower <- mutate_all(face_descriptions, .funs = tolower)
face_description_to_lower

```

# Using long_pivot() to reshape data into lang format

```{r}
face_descriptions_long <- face_descriptions %>%
  pivot_longer(starts_with("t"), names_to = "attribute_id", values_to = "description")
...
#Keeping first 3 responses only
```{r}
face_descriptions_split <- face_description_to_lower %>%
  select(1:6)
face_descriptions_split
...
#Converting above split data table to long format
```{r}
all_descriptions <- face_descriptions_split %>%
  pivot_longer(starts_with("t"), 
  names_to = "description_no", 
  names_prefix = "t", 
...
#separate description into multiple rows
```{r}
separated_face_descriptions <- all_descriptions %>% 
  separate_rows(description, convert = TRUE) # separate into multiple rows
separated_face_descriptions
```
...
```{r}
arranged_desc <- all_descriptions %>%
  group_by(description) %>%
  summarise(count = n()) %>%
  arrange(desc(count))  %>%
  head(10)
arranged_desc
#head(arranged_desc, 10)  
tail(arranged_desc, 10)
```
...
```{r}
top_10_descriptions <- all_descriptions %>%
  select(user_id, sex, age, description_no, description) %>%
  filter(description %in% arranged_desc$description) %>%
  #summarise(count = n()) %>%
  #summarise(count = n()) %>%
  arrange(desc(n()))
