...
```
Ans: Top brand is Apple with highest number of phones!
...
top_brand_stats <- mobile_phone_reviews %>%
  filter(brand == "Apple") %>% #we know that top brand is Apple
  summarise(
    num = n(),
    mean = mean(rating),
    sd = sd(rating)
...
top_brand_stats
```
The Apple brand has 101 sample points with mean rating of 3.52 with a 0.58 deviation from the mean
